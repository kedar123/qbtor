module ApplicationHelper
  ...
  def set_page_title(*title_elements)
    title_elements.unshift("Cheqbook") unless title_elements.first.starts_with?("Cheqbook")
  ...
  def page_title
    @_page_title || 'Cheqbook.com: Accounting simply done.'
  end
  ...
  def user_profile_link(user, options={})
    ...
    if options[:show_badge]
      name << "<br/>" << image_tag("wesabe-tiny.gif", :alt => "Works at Cheqbook", :title => "Works at Cheqbook", :class => "badge")
    end
  ...
  def logo
    link_to("Cheqbook.com: Accounting simply done.", root_url, :id => 'logo')
  end
end
